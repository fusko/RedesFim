...
sudo iptables -t nat -I PREROUTING -p tcp -d 192.168.122.1/24  --dport 80 -j DNAT --to-destination 127.0.0.1:80

#OUTPUT - Saída
#limitação de acesso ao Facebook - Drop de conexão
sudo iptables -A OUTPUT -d facebook.com -j DROP
#limitação de acesso ao Twitter - Drop de conexão
sudo iptables -A OUTPUT -d twitter.com -j DROP
